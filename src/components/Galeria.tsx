...
export default function Galeria() {
  ...
  return (
    <section
      ...
      className="w-full bg-[#F4F1EC] text-[#1A1814] pt-12 pb-24 md:pt-16 md:pb-32 border-b border-line-dark select-none overflow-hidden relative"
      style={{
    ...
    >
      ...
      <div 
        ...
        style={{
          backgroundImage: 'radial-gradient(rgba(246, 154, 79, 0.12) 1.2px, transparent 1.2px)',
          backgroundSize: '24px 24px',
        }}
      />

      {/* Soft Warm Identity Glow behind the composition (extremely soft radial gradient) */}
      <motion.div 
        className="absolute top-[50%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-[1000px] aspect-square rounded-full pointer-events-none z-0 opacity-90 blur-[140px]" 
        animate={{
          x: mousePos.x * -50,
          y: mousePos.y * -35,
        }}
        ...
        style={{
          background: 'radial-gradient(circle, rgba(246,154,79,0.18) 0%, rgba(14,114,129,0.06) 40%, rgba(244,241,236,0) 70%)'
        }} 
      />
      ...
      {/* Content wrapper with restricted width of 1250px */}
      <div className="max-w-[1250px] mx-auto px-6 md:px-12 relative z-10 flex flex-col items-center">
        
        {/* 3D PERSPECTIVE COVERFLOW CAROUSEL WITH DEPTH & INTEGRATED BACKDROP TITLE */}
        <div className="relative w-full h-[460px] md:h-[640px] flex items-center justify-center select-none overflow-visible">
          ...
          {/* Interactive 3D Card Elements Layer */}
          <div className="absolute inset-0 flex items-center justify-center overflow-visible mt-20 md:mt-24">
            {galleryData.map((item, index) => {
              ...
              const translateX = diff * (isMobile ? 110 : 290);
              
              // Scale: central is 25% bigger on desktop. Side cards scale progressively down with high contrast
              const scale = isActive 
                ? (isMobile ? 1.12 : 1.25) 
                : (isMobile ? 0.65 : 0.76) - absDiff * 0.06;
              
              const opacity = isActive ? 1.0 : (absDiff === 1 ? 0.65 : 0.2);
              const zIndex = 30 - absDiff;
              
              // Gorgeous Camera Lens Aperture Blur effect for surrounding cards
              const blurVal = isActive ? 0 : (absDiff === 1 ? 4 : 8);
              ...
              return (
                <motion.div
                  ...
                  animate={{
                    x: translateX + mousePos.x * (isActive ? 20 : 8),
                    y: mousePos.y * (isActive ? 12 : 5),
                    scale: scale,
                    opacity: opacity,
                    rotateY: diff * -18, // Smooth elegant 3D perspective orientation
                    z: isActive ? 120 : -140 * absDiff, // Depth feeling
                  }}
                  whileHover={isActive ? {
                    scale: isMobile ? 1.15 : 1.28,
                    y: -15,
                  ...
                  className={`relative w-[180px] h-[250px] md:w-[310px] md:h-[420px] rounded-[24px] overflow-hidden cursor-grab active:cursor-grabbing transition-shadow duration-500 will-change-transform
                    ${isActive 
                      ? 'shadow-[0_45px_95px_rgba(246,154,79,0.3),_0_20px_40px_rgba(26,24,20,0.22)] border-2 border-[#F69A4F]/40' 
                      : 'shadow-md border border-[#1A1814]/10'
                    }`}
                >
                  <img
                    src={item.image}
                  ...
                  {isActive && (
                    <div className="absolute inset-0 bg-black/25 opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <div className="flex flex-col items-center gap-2 transform translate-y-3 hover:translate-y-0 transition-transform duration-500">
                        <div className="w-10 h-10 rounded-full border border-white flex items-center justify-center text-white bg-black/35 backdrop-blur-md">
                          <Maximize2 size={16} />
                        </div>
                        <span className="text-[0.62rem] tracking-[0.25em] text-white/90 uppercase font-semibold">
                          Ampliar
                        </span>
                      </div>
                    </div>
        ...
        <div className="flex flex-col items-center gap-6 mt-12 md:mt-16 relative z-10 w-full">
          ...
          <div className="min-h-[80px] text-center px-6 max-w-2xl mx-auto flex items-center justify-center">
            <AnimatePresence mode="wait">
              <motion.p
                ...
                className="font-display italic text-[#1A1814]/85 text-base md:text-xl leading-relaxed font-light"
              >
      ...
      <AnimatePresence>
        {selectedImageIndex !== null && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/95 backdrop-blur-md z-[100] flex flex-col justify-between p-6"
          ...
          >
            ...
            <div className="flex items-center justify-between w-full relative z-10 max-w-7xl mx-auto">
              ...
              <button
                onClick={() => setSelectedImageIndex(null)}
                className="text-white/60 hover:text-white p-3 rounded-full border border-white/10 hover:border-white/45 transition-colors cursor-pointer"
            ...
            <div className="relative flex items-center justify-center h-[65vh] md:h-[70vh] w-full max-w-5xl mx-auto select-none">
              ...
              <button
                ...
                className="absolute left-0 z-10 text-white/40 hover:text-[#F69A4F] p-4 cursor-pointer hidden md:block"
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <img
                    ...
                    className="max-h-[60vh] object-contain border border-white/10 shadow-2xl rounded-lg"
                    referrerPolicy="no-referrer"
                    decoding="async"
                  />
                </motion.div>
              ...
              <button
                ...
                className="absolute right-0 z-10 text-white/40 hover:text-[#F69A4F] p-4 cursor-pointer hidden md:block"
            ...
            <div className="w-full text-center relative z-10 max-w-4xl mx-auto">
              <p className="font-display italic text-white text-sm md:text-base leading-relaxed font-light px-6">
              ...
              <div className="flex justify-center gap-8 mt-6 md:hidden">
                <button
                  ...
                  className="p-3 text-white border border-white/20 rounded-full"
                >
                  <ChevronLeft size={20} />
                </button>
                <button
